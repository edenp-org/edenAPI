...
public class TextModerationAutoRouteHelper
{
    ...
    public TextModerationResponse Examine(string text)
    {
        ...
        {
            role = "system",
            content = $"你现在是一个审核员,接下来我将给你一个HTML文本，请在选然后告诉我这个HTML文本是否包含淫秽内容、涉及敏感政治内容、是否有AI生成的可能性，如果包含请告诉我包含片段 结果请以 JSON 的形式输出，输出的 JSON 需遵守以下的格式：{resultTemplate.ToJsonString()}"
        });
